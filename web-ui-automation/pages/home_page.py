...
class HomePage(BasePage):
    ...
    def verify_left_sidebar_visible(self):
        return self.is_visible(self.LEFT_SIDEBAR)

    def verify_category_title_women_visible(self):
        return self.is_visible(self.CATEGORY_TITLE_WOMEN)

    def verify_category_title_men_visible(self):
        return self.is_visible(self.CATEGORY_TITLE_MEN)



    def verify_Logged_in_as_username_visible(self):
        if not self.is_visible(self.LOGGED_IN_USER):
            return False
        return self.get_text(self.LOGGED_IN_USER) == "Logged in as Ori Peles"
